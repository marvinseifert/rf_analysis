...
# %%
# FOR MOVING_BAR and MOVING_BAR_TINY (existing stimuli), the stimulus may not cover
# the full array. A recording can therefore optionally be restricted to a saved list
# of cells whose RF centres fall inside the stimulated region.
#
# Set central_cells_path inside each dataset config:
#   Path(...) = analyse only cells in that saved .npy list
#   None      = analyse all available cells for that recording
#
# For 15_05_2026:
#   300px list = moving bar
#   400px list = moving bar tiny
...
dataset_configs = [
    {
        ...
        "stimulus_index": 4,  # change moving bar stimulus here
    ...
    #     "central_cells_path": None,  # analyse all cells
    # },
    # {
...
single_cell_recording_id = "15_05_2026_p0"
single_cell_id = 12
...
def analyse_one_dataset(
    ...
    central_cells_path = config.get("central_cells_path", None)
    ...
    if central_cells_path is None:
        print("Central RF cell filter: OFF — analysing all available cells")
    else:
        print(f"Central RF cell filter: ON — {central_cells_path}")
    ...
    try:
        ...
        print(f"Number of cells in triggered spike data: {len(available_cells)}")

        # Optionally restrict this recording to the saved central-RF cell list.
        # When central_cells_path is None, every available cell is retained.
        if central_cells_path is not None:
            central_cells_path = Path(central_cells_path)

            if not central_cells_path.exists():
                raise FileNotFoundError(
                    f"Central-cell list was not found for {recording_id}:\n"
                    f"{central_cells_path}"
                )

            central_cell_ids = np.asarray(
                np.load(central_cells_path),
                dtype=int,
            )

            n_before_central_filter = len(available_cells)
            available_cells = available_cells[
                np.isin(available_cells, central_cell_ids)
            ]

            print("\nCentral RF cell filter:")
            print(f"Cells before filter: {n_before_central_filter}")
            print(f"Cell IDs in saved list: {len(central_cell_ids)}")
            print(f"Cells remaining after filter: {len(available_cells)}")

            if len(available_cells) == 0:
                raise ValueError(
                    f"No triggered cells in {recording_id} were present in "
                    f"the central-RF list:\n{central_cells_path}"
                )
        else:
            print("Central RF cell filter not applied.")

        print(f"Number of cells that will be analysed: {len(available_cells)}")
