...
from location.locate import project_radii_to_xy
...
def cutout_nans(data):
    # Compute mask of valid (non-NaN) pixels
    x_slice = slice(
        np.where(np.any(data > 0, axis=0))[0][0],
        np.where(np.any(data > 0, axis=0))[0][-1] + 1,
        1,
    )
    y_slice = slice(
        np.where(np.any(data > 0, axis=1))[0][0],
        np.where(np.any(data > 0, axis=1))[0][-1] + 1,
        1,
    )
...
path_to_data = Path(
    "/run/user/1000/gvfs/smb-share:server=mea_nas_25.local,share=root/"
    "Laura/zebrafish_02_12_2025/Phase_01/noise_analysis_test/noise_data.nc"
)
...
# %%
best_cell = sorted_cells[3].values
...
for c_idx, channel in enumerate(dataset.channel.values):
    ...
    axs[c_idx, 2].plot(np.deg2rad(radii.coords["degree"].values), radii.values)
    ...
    axs[c_idx, 3].plot(np.deg2rad(radii.coords["degree"].values), radii.values)
    ...
    # in out outline
    best_cell_data.sel(channel=channel)["in_out_outline_um"].plot.line(ax=axs[c_idx, 4])
    ...
    best_cell_data.sel(channel=channel)["sta_single_pixel"].plot.line(ax=axs[c_idx, 5])
    ...
    if c_idx == 0:
        ...
        axs[c_idx, 2].set_title(f"center outline", y=1.5)
