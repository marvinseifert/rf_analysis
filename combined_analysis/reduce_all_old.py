...
def sta_2d_cov_collapse(
    ...
    for cell_pos, cell_idx in tqdm(
        ...
        for channel_index, channel in enumerate(recording_config.channel_names):
            ...
            # 8. Calculate RMS
            stimulus_id = int(
                Path(channel_data["sta_path"].item()).parts[-3].split("_")[-1]
            )
    ...
    # Calculate the size in um of the complete sta image. All cutouts from all channels will fit in this space.
