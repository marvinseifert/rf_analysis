...
root_path = pathlib.Path(rf"F:\Laura\zebrafish_15_01_2026\Phase_00")
...
noise_roots = [root_path / "4px_20Hz_40mins_shuffle_365_413_idx_4"]
# %%
quality_paths = [np.load(path / "quality.npy", mmap_mode="r") for path in noise_roots]
...
for noise_idx, noise_root in enumerate(noise_roots):
    ...
    nr_of_spikes = recording.spikes_df.query(
        f"stimulus_index=={noise_parameters[noise_idx]['noise_stim_index']}"
    )["nr_of_spikes"].values
    for cell_idx in tqdm.tqdm(range(nr_cells), smoothing=0):
        ...
        if (
            np.sum(mask) != cut * cut
        ):  # if the mask is smaller than the cut size, e.g. if the cell is close to the edge
            subset_dev = np.max(
                (subset_flat / nr_of_spikes[cell_idx] - 0.5) ** 2, axis=0
            ).reshape(mask_shape)
        ...
        cell_params["quality_analysis"]["max_quality_channel"] = int(
            max_quality_channel
        )
        cell_params["quality_analysis"]["position"] = (
            int(position[0]),
            int(position[1]),
        )
