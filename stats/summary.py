...
def calculate_stats(
    ...
    # %% Calculate RF size
    center_sizes = [
        calculate_radial_area(outline) for outline in df["center_outline"].to_list()
    ]
    surround_sizes = [
        calculate_radial_area(outline) for outline in df["surround_outline"].to_list()
    ]
    sizes = [
        recording_config.channel_configs[ch].pixel_size
        for ch in df["channel"].to_list()
    ]
    center_sizes_mm = np.asarray(center_sizes) * np.asarray(sizes) ** 2 * 1e-6
    surround_sizes_mm = np.asarray(surround_sizes) * np.asarray(sizes) ** 2 * 1e-6
    df = df.with_columns(
        pl.Series("center_size_mm2", center_sizes_mm),
        pl.Series("surround_size_mm2", surround_sizes_mm),
    )

    # %% calculate RF roundness
    # project radii to x,y
    ...
    positions = [
        project_radii_to_xy(outline) for outline in df["center_outline"].to_list()
    ]
    tirs = []
    center_shifts = []
    tilts = []
    ...
    for pos in positions:
        ...
        radial_distances = np.sqrt((pos[:, 0] - xc) ** 2 + (pos[:, 1] - yc) ** 2)
    ...
    df = df.with_columns(
        pl.Series("center_tir", tirs),
        pl.Series("center_shift", center_shifts),
        pl.Series("center_tilt", tilts),
        pl.Series("center_angle_deg", angles),
    )
